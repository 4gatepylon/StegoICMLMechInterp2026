...
- [ ] 5. `NOT IMPORTANT`  Support a mock model that can work on special code files with UUID substrings (or something like that) and output canned correct/incorrect responses. This should enable us to run the harness fully (fully integration test). Ideally this mock model can run on an OpenAI API-compatible endpoint. Ideally, I can also support local (tiny) models via Ollama or something like that (even HF is fine; I could copy over my batched HF server from SAE Scoping old code---any agents working on this should ask user for that if needed; but we prefer VLLM or SGLang or Ollama or other local LLM server runtimes).
- [x] 6. Support running unit tests on modal reliably/determining whether code runs/works. (APPS evaluator with fresh sandboxes, timeouts, cleanup, and live reference/generated-solution checks; see [`data/README.md`](ciphers/variable_naming_in_python_v2/data/README.md).)
- [x] 7. Support generating responses/code from an LLM at all. (Subscription-backed Codex SDK: live basic questions and APPS Python generation in [`generate_apps_codex_modal.ipynb`](ciphers/variable_naming_in_python_v2/data/generate_apps_codex_modal.ipynb).)
- [x] 8. Implement evaluation setup harness without LLM Judges (only encoder/decoder). (Notebook prototype: N Codex samples, Modal functional tests, exact V2 decoding, and separate functional/message/joint pass@k. Live demo: 3/3 secret solutions passed all 10 tests and recovered `101`.)
- [x] 9. Implement and evalute MVP v1 harness. This harness will just try 1-shot and either fail or succeed. Support both OpenRouter and Codex directly (add a TODO for OpenAI API and/or Anthropic/Claude, mentioning use of max plan; maybe support generic OpenAI API-compatible endpoints). (Codex one-shot path is demonstrated in the notebook; OpenRouter remains outstanding.) `NOTE: this is implemented only as a experiment on small number of samples, not as a pipeline.` 
- [ ] 10. `NOT IMPORTANT` Define a harness API and implement and evaluate MVP v2 harness. This harness will 2-shot (first one produces the code, second one modifies it).
- [ ] 11. `NOT IMPORTANT` Clean up and generalize harnesses. All of them must take in generic multi-evaluator (intersection) stuff. Define final harness with everything exept reward-hacking judge and only using most basic "black box detection" judge (the "Basic" one). We can work with Luna for now.
