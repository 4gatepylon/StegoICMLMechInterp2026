...
for GPT-OSS-120B, then separately for GPT-OSS-20B. For 100 inputs, each model runs
batches of 32, 32, 32, and 4. After saving both models' answers, it grades them
...
- `gpt-oss-120b.jsonl`, `gpt-oss-20b.jsonl`, `gpt-5.6-luna.jsonl`: raw answers and errors.
...
- `openrouter-timings.json`, `codex-timings.json`: preparation/loading, generation, grading and total wall times.
...
sampling defaults apply. The shared cipher uses one control bit and four length
