...
from ciphers.kirchenbauer_et_al.src.extract_kl_fineweb import probability_of_bit_deprecated
...
def probability(text: str, bit: int, model: FixedBaseModel) -> float:
    return probability_of_bit_deprecated(text, bit, model, Tokenizer(), RED, GREEN, DELTA)
...
def test_red_and_green_must_partition_the_vocabulary(uniform_model: FixedBaseModel) -> None:
    with pytest.raises(AssertionError, match="cover every vocabulary token exactly once"):
        probability_of_bit_deprecated("all red", 1, uniform_model, Tokenizer(), RED[:-1], GREEN, DELTA)
