...
def get_hydrating_dict(dataset_entry: Dict[str, str]) -> Dict[str, str]:
    ...
    assert isinstance(dataset_entry["answers"], (list, str)), (
        f"type={type(dataset_entry['answers'])}\n\n{dataset_entry['answers']}"
    )
    ...
    assert isinstance(dataset_entry["answers"], list), (
        f"type={type(dataset_entry['answers'])}\n\n{dataset_entry['answers']}"
    )
    answer = (
        random.choice(dataset_entry["answers"])
        if len(dataset_entry["answers"]) > 0
        else "No answer was provided... (oops!)"
    )
    ...
    assert isinstance(inputs_outputs, dict), (
        f"inputs_outputs is not a dict: {type(inputs_outputs)}\n\n{inputs_outputs}"
    )
    assert {"inputs", "outputs"}.issubset(set(inputs_outputs.keys())), (
        f"inputs_outputs keys is {set(inputs_outputs.keys())}"
    )
    ...
    inputs_outputs_string = "\n".join(
        json.dumps({"input": i, "output": o}, indent=4) for i, o in zip(inputs, outputs)
    )
    ...
    assert set(keys_to_keep).issubset(set(output.keys())), (
        f"output keys is {set(output.keys())}"
    )
    returnmeplz = {k: output[k] for k in keys_to_keep}
    _types_dict = {k: type(v) for k, v in returnmeplz.items()}
    assert all(isinstance(v, str) for v in returnmeplz.values()), (
        f"types dict: {_types_dict}"
    )
    return returnmeplz
